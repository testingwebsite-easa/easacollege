...
function parseSequentialElements(html) {
    const elements = [];
    const blockRegex = /<(p|h1|h2|h3|h4|h5|h6|table)[^>]*>([\s\S]*?)<\/\1>/gi;
    let match;
        ...
        if (tag === 'table') {
            ...
            while ((rowMatch = rowRegex.exec(content)) !== null) {
                ...
                while ((cellMatch = cellRegex.exec(rowHtml)) !== null) {
                    const cellText = cleanText(cellMatch[1].replace(/<[^>]*>/g, ' '));
                    cells.push(cellText);
                }
...
/**
 * Robust Detailed Syllabi Parser (Units, Objectives, Outcomes, Textbooks, References)
 */
 ...
 * @param {Buffer} buffer Word Document buffer (.docx)
 */
async function parseWordSyllabus(buffer) {
    try {
        ...
        const elements = parseSequentialElements(html);
