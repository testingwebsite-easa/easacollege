...
const FacultyHandbookPage = () => {
    const [showAdmissionForm, setShowAdmissionForm] = useState(false);
    const [resources, setResources] = useState([]);
    const [loading, setLoading] = useState(true);
    const title = "Faculty Handbook";
    const subtitle = "Guidelines, code of conduct, and resources for faculty members.";
            ...
            <section className="container" style={{ padding: '4rem 2rem', flex: 1, position: 'relative', zIndex: 10 }}>
                {loading ? (
                        ...
                        {resources.map((resource, index) => (
                            <motion.div key={resource._id || index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="glass-card" style={{ padding: '2rem', border: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                ...
                                <div style={{ marginTop: 'auto', paddingTop: '1rem' }}>
                                    {resource.fileUrl ? (
                                        <a href={resource.fileUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ display: 'inline-block', padding: '0.5rem 1rem', fontSize: '0.9rem' }}>Download Handbook</a>
                    ...
                    <div className="glass-card" style={{ padding: '3rem', minHeight: '300px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                        ...
                        <h2 style={{ marginBottom: '1rem' }}>Handbook Unavailable</h2>
                        <p style={{ color: 'var(--text-muted)', maxWidth: '600px', lineHeight: '1.6' }}>The faculty handbook is being revised.</p>
                        <button className="btn btn-primary" style={{ marginTop: '2rem' }} onClick={() => setShowAdmissionForm(true)}>Contact HR</button>
                    </div>
                )}
            </section>
...
export default FacultyHandbookPage;
