...
const StatutoryBodiesPage = () => {
    const [showAdmissionForm, setShowAdmissionForm] = useState(false);
    const [resources, setResources] = useState([]);
    const [loading, setLoading] = useState(true);
    const title = "Statutory Bodies";
    const subtitle = "Information about Governing Body, Academic Council, and Boards.";
    ...
    return (
        <div className="resource-page" style={{ position: 'relative', overflowX: 'hidden', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <section className="container" style={{ padding: '4rem 2rem', flex: 1, position: 'relative', zIndex: 10 }}>
                {loading ? (
                    <div style={{ textAlign: 'center', padding: '2rem' }}>Loading statutory bodies info...</div>
                ) : resources.length > 0 ? (
                        ...
                        {resources.map((resource, index) => (
                            <motion.div key={resource._id || index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="glass-card" style={{ padding: '2rem', border: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                ...
                                <div style={{ marginTop: 'auto', paddingTop: '1rem' }}>
                                    {resource.fileUrl ? (
                                        <a href={resource.fileUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ display: 'inline-block', padding: '0.5rem 1rem', fontSize: '0.9rem' }}>View Details</a>
                                    ) : <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>No document attached</span>}
                                </div>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    <div className="glass-card" style={{ padding: '3rem', minHeight: '300px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                        ...
                        <h2 style={{ marginBottom: '1rem' }}>Information Unavailable</h2>
                        <p style={{ color: 'var(--text-muted)', maxWidth: '600px', lineHeight: '1.6' }}>Details about statutory bodies are currently being updated.</p>
                        <button className="btn btn-primary" style={{ marginTop: '2rem' }} onClick={() => setShowAdmissionForm(true)}>Contact Administration</button>
                    </div>
                )}
            </section>
