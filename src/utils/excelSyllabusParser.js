...
/**
 * High-Precision Excel Syllabus & Curriculum Parser
 * Supports .xlsx, .xls, and .csv files with multi-sheet and single-sheet alignment
 */
...
function inferSemesterFromCourseCode(code) {
    ...
    // 2. Anna University 2021 Regulation (e.g. CS3351 -> 3, MA3151 -> 1, GE3251 -> 2, IT3401 -> 4, CS3591 -> 5, CS3691 -> 6, CS3701 -> 7, CS3801 -> 8)
    ...
    // 3. Anna University 2017 Regulation (e.g. CS8351 -> 3, CS8491 -> 4, CS8591 -> 5, CS8691 -> 6, CS8791 -> 7, CS8811 -> 8, GE8151 -> 1, MA8251 -> 2)
...
function detectSheetContext(sheetName) {
    ...
    // Detect Semester / Session Sheet (e.g. "Sem 1", "Semester I", "Sem-4", "SEM 5", "Session 3", "Session-2", "SEM3")
    ...
    // Detect Vertical Sheet (e.g. "Vertical 1", "PE Vertical 2", "Track 3")
...
/**
 * Scan a 2D sheet array to extract subject rows with flexible column header alignment
 */
function parseSheetRows(rows, defaultContext) {
    ...
    for (let r = 0; r < rows.length; r++) {
        ...
        // Skip credit summary / footer tables
        ...
        // Check for Vertical section banner in sheet
        ...
        // Check for Open Elective section banner
        ...
        // Department header inside Open Electives
        ...
        // Try detecting column header row
        ...
        if (hasCodeHeader || (hasTitleHeader && (rowUpper.includes('L') || rowUpper.includes('C') || rowUpper.includes('CREDITS')))) {
            ...
            rowUpper.forEach((col, idx) => {
                ...
                else if (c.includes('OBJECTIVE')) colMap.objectives = idx;
                else if (c.includes('OUTCOME')) colMap.outcomes = idx;
            });
            continue;
        }

        // Try extracting row data
        let code = '';
        let title = '';
        let rawCategoryVal = '';
        ...
        // Fallback: positional column search if colMap wasn't set or code wasn't matched
        if (!code) {
            let codeIdx = -1;
            ...
            if (codeIdx !== -1) {
                ...
                // Title is usually adjacent to code
                ...
                // Extract numerical values using sequence parser
        ...
        if (code) {
            ...
            // Align CIA/ESE marks defaults according to category
            ...
            // Semester inference if not defined
            ...
            // Generate structured Outcomes & Units
...
/**
 * Main Client-side Parser for Excel (.xlsx, .xls, .csv) files or ArrayBuffers
 * @param {File|ArrayBuffer} fileOrBuffer
 */
export async function parseExcelSyllabusClient(fileOrBuffer) {
    try {
        let arrayBuffer;
        if (fileOrBuffer instanceof ArrayBuffer) {
            ...
            throw new Error('Invalid file format. Please provide an Excel spreadsheet (.xlsx, .xls, .csv).');
        }
        ...
        console.error('Client Excel Parser Error:', error);
        return {
            success: false,
            error: error.message || 'Failed to parse Excel spreadsheet',
            subjects: []
        };
    }
}
